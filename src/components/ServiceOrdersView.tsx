...
export default function ServiceOrdersView({ 
  ...
  const [isBulkReverting, setIsBulkReverting] = useState<boolean>(false);
  ...
  return (
    <div className="space-y-6 font-sans">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-150/70" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex justify-end items-center gap-1.5">
                      {isNotExecuted && userProfile?.perfil !== 'Profissional' && (() => {
                        // Checar se esta OS específica ainda está dentro do prazo da criação em lote
                        const canRevertThis = canRevertUnexecutedOrder(os, currentCalendarDate);
                        if (!canRevertThis) {
                          return (
                            <span 
      ...
      {/* BULK REVERT CONFIRMATION MODAL FOR NÃO EXECUTADAS (Somente dentro do prazo do Super Adm) */}
      {showBulkRevertModal && (() => {
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md w-full border border-amber-200 transform transition-all animate-in fade-in zoom-in-95 duration-150 text-left">
              ...
              <div className="space-y-3 mb-6">
                <p className="text-xs text-slate-700 leading-relaxed font-medium">
                  Você está prestes a reverter <strong className="font-black text-amber-800">{unexecutedList.length}</strong> {unexecutedList.length === 1 ? 'preventiva não executada' : 'preventivas não executadas'} válidas dentro do SLA de volta para o status <strong className="font-bold text-indigo-700">"Novo"</strong>.
                </p>
