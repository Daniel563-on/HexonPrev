  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      {userProfile?.perfil !== 'Profissional' && (
        <div className="flex bg-white p-1 rounded-xl border border-slate-200 shadow-xs max-w-md">
          <button
            type="button"
            onClick={() => setSubTab('planejamento')}
            className={`flex-1 py-2.5 px-4 rounded-lg text-xs font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-200 cursor-pointer ${
              subTab === 'planejamento'
                        ...
                        <div className="bg-white p-3.5 rounded-xl border border-slate-150 space-y-3 shadow-2xs">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            {userProfile?.perfil === 'Super Administrador' || userProfile?.gerencia === 'Todas' ? (
                              <select
                                value={planSector}
                                onChange={(e) => setPlanSector(e.target.value)}
                                className="text-xs p-1.5 bg-slate-55 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-500 font-bold text-[#3525cd]"
                              >
                                <option value="all">Setor: Todos</option>
                                <option value="HVAC">HVAC / Climatização</option>
                            ...
                            <div className="md:col-span-2 flex items-center gap-2 bg-indigo-50/40 p-2 rounded-lg border border-indigo-100">
                              <input
                                type="checkbox"
                                    ...
                                    <button
                                      ...
                                      onClick={async () => {
                                        ...
                                        try {
                                          ...
                                          onReload();
                                          alert(`Preventiva #${os.id} agendada para o dia ${selectedCalendarDay}/${currentCalendarDate.getMonth() + 1} com sucesso!`);
                                        } catch (error) {
